...
public class YahooFinanceStockFetcherTest
{
	...
	public async Task YahooFinanceStockFetcherTest_GetHistory_SuccessfulTest()
	{
		...
		{
			Data.StockHistory stockHistory = await stockFetcher.GetHistory(stock.Value, stock.Key, DateOnly.Parse("2021-01-01"), DateOnly.Parse("2022-01-01"), "daily");
	...
	{
		Data.StockHistory stockHistory = await stockFetcher.GetHistory("invalid", "NASDAQ", DateOnly.Parse("2021-01-01"), DateOnly.Parse("2022-01-01"), "daily");
	...
	{
		Data.StockHistory stockHistory = await stockFetcher.GetHistory("AAPL", "NASDAQ", DateOnly.Parse("2021-02-01"), DateOnly.Parse("2021-01-01"), "daily");
		Assert.IsTrue(stockHistory.history.Count == 0, "History should be empty for invalid ticker");
	}
