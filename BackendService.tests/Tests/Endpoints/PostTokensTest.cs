...
{
	private static StockApp.User user = new StockApp.User("", "");
	...
	{
		PostTokensBody body = new PostTokensBody(user.email!, user.password!);
	...
	public void PostTokensTest_UserNotExistsTest()
	{
		...
		PostTokensBody body = new PostTokensBody(email, user.password!);
	...
	public void PostTokensTest_WrongPasswordTest()
	{
		...
		PostTokensBody body = new PostTokensBody(user.email!, password);
		StatusCodeException exception = Assert.ThrowsException<StatusCodeException>(() => PostTokens.Endpoint(body));
	...
	{
		PostTokensBody body = new PostTokensBody("", user.password!);
		StatusCodeException exception = Assert.ThrowsException<StatusCodeException>(() => PostTokens.Endpoint(body));
	...
	{
		PostTokensBody body = new PostTokensBody(user.email!, "");
		StatusCodeException exception = Assert.ThrowsException<StatusCodeException>(() => PostTokens.Endpoint(body));
		Assert.IsTrue(exception.StatusCode == 400, "StatusCode should be 400 but was " + exception.StatusCode);
